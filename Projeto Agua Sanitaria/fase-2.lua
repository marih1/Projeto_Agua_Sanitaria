...
function scene:create( event )
    ...
    local botaoPular = display.newRect(sceneGroup, w,h,640,720);
    ...
    local background = display.newImageRect(sceneGroup, "image/background.png", w, h);
    ...
    local plataformas = {}
    --PLATAFORMA 1
    plataformas[1] = display.newRect(sceneGroup,w,h,1280,60);
    ...
    local pontos = {}
    pontos[1] = display.newCircle(sceneGroup, w * .1, plataformas[1].y - 100, 20);
    physics.addBody(pontos[1], "static", {bounce=0, friction = 0});
    pontos[1].objType = "score"
    pontos[2] = display.newCircle(sceneGroup, w * .8, plataformas[1].y - 100, 20);
    physics.addBody(pontos[2], "static", {bounce=0, friction = 0});
    pontos[2].objType = "score"

    --BOTÕES PARA MOVIMENTAÇÃO
    local botoes = {}
    botoes[1] = display.newImageRect(sceneGroup, "image/arrow.png", 140, 140);
    botoes[1].x, botoes[1].y = w * .2, h * .88
    botoes[1].name = "right"
    botoes[2] = display.newImageRect(sceneGroup, "image/arrow.png", 140, 140);
    ...
    local congratulations = display.newText(options);
    sceneGroup:insert(congratulations)
    congratulations:setFillColor(1,0,0) 
    ...
    local function jump(e)
        ...
        if e.phase == "began" or e.phase == "moved" then
            ...
            if e.target.name == "botaoPular" and sensor == 0 then
                ...
                mc:applyLinearImpulse( nil, -1.4, mc.x, mc.y )
                --pessoa:setSequence("andandoDireita")
            end
        elseif e.phase == "ended" or e.phase == "cancelled" then
            --code
